...
morphed_images = [cv2.bitwise_and(image, image, mask=morphed_mask) for image, morphed_mask in
                  zip(images, morphed_masks)]
...
for i in range(len(sliced_image_list)):
    ...
    for j in range(len(sliced_images)):
        ...
        if (exist):
            ...
            debug_label = basic.draw_debug_info(sliced_image, contour_max, left, right, top, bottom, width, height,
                                                color, double_count)
            if (debug_flag):
                np.save(config.temp_path + image_names_list[i].rsplit(".", 1)[0] + '_sliced_mask_' + str(
                    j // 6) + '_' + str(j % 6) + '.npy', sliced_mask, allow_pickle=True)
                cv2.imwrite(config.temp_path + image_names_list[i].rsplit(".", 1)[0] + '_sliced_image_' + str(
                    j // 6) + '_' + str(j % 6) + '.png',
                            cv2.cvtColor(basic.extract_img(osi, sliced_mask), cv2.COLOR_BGR2RGB))
                subfig.savefig(config.temp_path + image_names_list[i].rsplit(".", 1)[0] + '_sliced_mask_' + str(
                    j // 6) + '_' + str(j % 6) + '.png')
                plt.close(subfig)
        even_num_j = j % 2
        if (even_num_j == 0):
